...
public class ActivityController {
	...
	public ModelAndView indexActivity(HttpServletRequest request,
		...
		List<User> friendList = userDao.getAllFriends(userId);
...
flag=1;
	...
	public ModelAndView activityInfo(HttpServletRequest request,
		...
		User currentUser=(User)session.getAttribute("user");//得到当前用户对象
		if(currentUser!=null){
			if(currentUser.getUserId()==establishUser.getUserId())//说明当前用户就是此活动发起用户 所以有图片上传的权限
			{flagg=1;
	...
	public ModelAndView addPicIndex(HttpServletRequest request,
		...
		Integer flag=0;
		User user=userDao.getUserById(activityDao.getActivityById(activityId).getUserId());//得到照片上传者对象
		HttpSession session=request.getSession();
		
		User currentUser=(User)session.getAttribute("user");//得到当前用户对象
		if(currentUser!=null){
			if(currentUser.getUserId()==user.getUserId())//说明当前用户就是此活动发起用户 所以有图片上传的权限
			{flag=1;
...
if(activityAlbumDao.getActivityAlbumByCondition(0, albumId)==null){//如果相册查询 结果为空
	...
	String albumName=request.getParameter("albumName");
		...
		view.setViewName("redirect:showPic.action?activityPicId="
				+ activityPicId+"&albumName="+albumName+"&activityId="+activityId);// 设置对应的视图
		return view;
	...
	public ModelAndView showPic(HttpServletRequest request,
		...
		while (iteratorComment.hasNext()) {
			...
			userComment.setUserName(userDao.getUserById(
					activityPicComment.getUserId()).getUserAlias());
			userComment.setUserPic(userDao.getUserById(
					activityPicComment.getUserId()).getUserPic());// VO对象封装结束
	...
	public ModelAndView searchByTag(HttpServletRequest request,
		...
		//
		// while (UserComment.size() < 6) {
		// UserComment.add(null);
		//
		// }05/18/2012
		//
		// while (followList.size() < 8) {
		// followList.add(null);
		// }05/18/2012
		List result = new ArrayList();// 最终的结果集
